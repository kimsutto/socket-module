...
int CTCPClient::SetSendAddress(const char* IP, int port)
{
	int status = m_Client->connectToServer(IP, port);
	return status;
}

void CTCPClient::Send(char* sendmsg, int& dataSize)
{
	m_Client->write(sendmsg, &dataSize, QUANTAnet_tcpClient_c::BLOCKING); // send data
}

int CTCPClient::Receive(char* recvline, int& dataSize, int blockingType)
{
	int status = m_Client->read(recvline, &dataSize, blockingType); // receive data
	return status;
}
